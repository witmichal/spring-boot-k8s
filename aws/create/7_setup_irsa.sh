...
export SERVICES_ACCOUNT_NAME="irsa-s3-reader-s-a"
export IAM_ROLE_NAME="S3-Reader-Role-For-Irsa"
...
echo "Policies attached to $IAM_ROLE_NAME role"
aws iam list-attached-role-policies \
--role-name $IAM_ROLE_NAME \
| jq

echo "Trust relationship of $IAM_ROLE_NAME role"
...
kubectl annotate serviceaccount "$SERVICES_ACCOUNT_NAME" \
-n "$TEST_NAMESPACE" \
eks.amazonaws.com/role-arn="$IAM_ROLE_ARN"

kubectl describe sa -n "$TEST_NAMESPACE" "$SERVICES_ACCOUNT_NAME" # just test output
